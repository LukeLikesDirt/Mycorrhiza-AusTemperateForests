...
# ──────────────────────────────────────────────────────────────────────────────
#  Testing the effect of mineral nitrogen on EMF (ectomycorrhizal fungal)
#  alpha diversity using a causal modelling framework
#
#  Metrics modelled:
#  - Richness, Faith's PD, Hill taxonomic (q=1, q=2), Hill phylogenetic (q=1,
#    q=2), and EMF SRS abundance
#
#  Richness and Faith's PD are modelled on the raw scale here (not sqrt()-
#  transformed, unlike every response in 01a_alpha_diversity_amf.R and unlike
#  this script's own other five responses) -- see the AME back-transform note
...
#  so this script and 01a_alpha_diversity_amf.R both source them.
#
...
# holds for 01a_alpha_diversity_amf.R but NOT for this script: richness and
# Faith's PD below are modelled untransformed, so squaring an
# already-untransformed prediction would silently misreport -- exactly what
# the plan's assertion exists to prevent. response_backtf_for()
...
# safety property the plan asks for is preserved per-response. In this script
# richness and faith resolve to the identity function; the other five to eta^2.
...
data <- inner_join(
  ...
  mutate(
    ...
    # Tree covariates. ecm_tree_richness_std / ecm_tree_basal_area_std name
    # the *trees* that form ectomycorrhizas, not the fungi -- "EMF" is a
    # fungal label and does not belong on these variables (edit.pdf §9.2, the
    # same rule as am_tree_richness_std in 01a_alpha_diversity_amf.R). Any
    # displayed label for these two covariates should read "ectomycorrhizal
    # trees", not "ECM trees" or "EMF trees".
    ecm_tree_richness_std    = std(ecm_tree_richness),
    ecm_tree_basal_area_std  = std(
      pmax(ecm_tree_basal_area, quantile(ecm_tree_basal_area, 0.025))
    ),
...
dir.create("output/emf", showWarnings = FALSE, recursive = TRUE)
...
ggsave(
  "output/emf/covariates_alpha_diversity.png",
  width = 44, height = 44, units = "cm", dpi = 300
)

#### (2b) Relationships ####
# Exploratory response ~ covariate panels: linear, quadratic and loess fits.
# Inspect output/emf/ to decide which covariates need quadratic terms.
...
responses_explore <- list(
  ...
  list(var = "abundance_srs",   label = "EMF abundance")
)
...
for (resp in responses_explore) {
  ...
  ggsave(
    sprintf("output/emf/exp_%s.png", resp$var),
    plot = p,
    width = 18, height = n_rows * 6 + 3, units = "cm", dpi = 200
  )
  cat(sprintf("  Saved: output/emf/exp_%s.png\n", resp$var))
}

#### (2c) Polynomial term specification ####
#
# Inspect the panels in output/emf/ and set use_quadratic = TRUE for
# any covariate whose relationship with that response looks unimodal / curved.

covariates <- names(data) %>% str_subset("_std$")

# ── Set quad_covs for each response after inspecting output/emf/ ──────
...
# from 01a_alpha_diversity_amf.R (ROPE plan §13, step 9), with two EMF-specific
# differences preserved exactly from the pre-ROPE script: richness and Faith's
# PD are modelled untransformed here (AMF models both on the sqrt scale), and
# only abundance carries a quadratic term (bio1_std) — AMF instead puts a
# quadratic on ph_std in every response.
#
...
responses <- list(
  list(key = "richness",      var = "richness",        lhs = "richness",
       label = "Richness",          diag_label = "RICHNESS",
       tag = "(**a**)", poly_sel = poly_sel_richness),
  list(key = "faith",         var = "faith_phylo_div",  lhs = "faith_phylo_div",
...
# SD of each response on the scale it was modelled (e.g. sqrt(hill_tax_q1), or
# richness itself since it is untransformed here) — the constant that turns a
# partial coefficient into a fully standardised one (ROPE plan §3-4). Read
# from each model's own frame, not data$<var>, so NA-dropped rows are handled
# automatically.
...
# note near the top of this script) — identity for richness/faith, eta^2 for
...
# generated_data/boot_cache/emf_*.rds first.
...
for (r in responses) {
  cache_file <- file.path(
    boot_cache_dir,
    sprintf("emf_%s_seed%d_n%d.rds", r$key, SEED, N_BOOT)
  )
...
write_xlsx(
  ...
  path = "output/emf/coefficients_alpha_diversity.xlsx"
)
...
# state that beta is fully standardised (ROPE plan §9 "axis mismatch"). Faith's
# PD / Hill phylogenetic (q=1, q=2) get the beta1/beta2 annotation form
# automatically (get_boot_annotation() branches on whether a quadratic term
# exists for the parameter), unaffected by the AME work above.
...
#### (7c) Hill taxonomic diversity (q=1) plots ####
...
#### (7d) Hill phylogenetic diversity (q=1) plots ####
...
#### (7e) Hill taxonomic diversity (q=2) plots ####
...
#### (7f) Hill phylogenetic diversity (q=2) plots ####
...
# Rename all objects with _emf suffix so they can be loaded alongside
# analogous AMF objects without name collisions
data_emf                      <- data

pred_richness_nitrogen_emf    <- pred_richness_nitrogen
annot_rich_nitrogen_emf       <- annot_rich_nitrogen
r2_richness_emf               <- boot_richness$r2
partial_r2_richness_emf       <- boot_richness$partial_r2

pred_abund_srs_nitrogen_emf   <- pred_abund_srs_nitrogen
annot_abund_srs_nitrogen_emf  <- annot_abund_srs_nitrogen
r2_abundance_emf              <- boot_abundance_srs$r2
partial_r2_abundance_emf      <- boot_abundance_srs$partial_r2

if (!dir.exists("generated_data")) dir.create("generated_data")

save(
  data_emf,
  pred_richness_nitrogen_emf,
  annot_rich_nitrogen_emf,
  r2_richness_emf,            partial_r2_richness_emf,
  pred_abund_srs_nitrogen_emf,
  annot_abund_srs_nitrogen_emf,
  r2_abundance_emf,           partial_r2_abundance_emf,
  file = "generated_data/figure_2_emf.Rdata"
)
cat("Figure 2 EMF data saved to generated_data/figure_2_emf.Rdata\n")

#### Figure s6 ####

figure_s6 <- wrap_plots(
...
cat("Results exported to: output/emf/coefficients_alpha_diversity.xlsx\n")
...
cat("Figure 2 data saved to: generated_data/figure_2_emf.Rdata\n")
