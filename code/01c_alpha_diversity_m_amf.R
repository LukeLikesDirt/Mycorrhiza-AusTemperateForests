...
# ––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––
#  Testing the effect of mineral nitrogen on Densosporales alpha diversity using a
#  causal modelling framework
#
#  Metrics modeled:
#  - Richness, Faith's PD, Hill taxonomic (q=1, q=2), Hill phylogenetic (q=1,
#    q=2), and Densosporales SRS abundance
#
...
#
#  IMPORTANT: unlike the other three scripts, mineral_nitrogen_std (the focal
#  term) carries a quadratic component here for Faith's PD, Hill phylogenetic
#  (q=1) and Hill phylogenetic (q=2) — see poly_sel_faith / poly_sel_hill_phy /
#  poly_sel_hill_phy_q2 below. Per plan §10, a single linear beta is not a
#  valid summary of "the nitrogen effect" for those three responses, so their
#  Summary/Sensitivity rows report "quadratic — see per-response sheet" rather
#  than a beta and classification. The per-response sheets still classify
#  beta1 and beta2 separately, for transparency, exactly as any other
#  quadratic covariate (e.g. ph_std) is classified throughout these scripts.
# ––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––
...
# holds for every response in *this* script (01c), and in 01a/01b, but not for
...
data <- inner_join(
  fread("data/amf/diversity_m_amf.txt"),
...
dir.create("output/m_amf", showWarnings = FALSE, recursive = TRUE)
...
ggsave(
  "output/m_amf/covariates_alpha_diversity.png",
...
# Inspect output/m_amf/ to decide which covariates need quadratic terms.
...
responses_explore <- list(
  ...
  list(var = "abundance_srs",   label = "Densosporales abundance")
)
...
for (resp in responses_explore) {
  ...
  ggsave(
    sprintf("output/m_amf/exp_%s.png", resp$var),
    plot = p,
    width = 18, height = n_rows * 6 + 3, units = "cm", dpi = 200
  )
  cat(sprintf("  Saved: output/m_amf/exp_%s.png\n", resp$var))
}

#### (2c) Polynomial term specification ####
#
# Inspect the panels in output/m_amf/ and set use_quadratic = TRUE for
# any covariate whose relationship with that response looks unimodal / curved.

covariates <- names(data) %>% str_subset("_std$")

# ── Set quad_covs for each response after inspecting output/m_amf/ ──────
...
# from 01a_alpha_diversity_amf.R (plan §13, step 9 — extended here to M-AMF for
...
# FOCAL_TERM is quadratic for Faith's PD / Hill phylogenetic (q=1) / Hill
# phylogenetic (q=2) here — unlike 01a/01b/01d, where it is always linear.
# build_summary_row() and build_sensitivity_rows() (functions_alpha_diversity.R)
# detect this per response and report "quadratic — see per-response sheet"
# instead of a single linear beta, per plan §10. This is informational, not a
# hard stop, because that degradation path exists and is exercised below.
...
for (r in responses) {
  cache_file <- file.path(
    boot_cache_dir,
    sprintf("m_amf_%s_seed%d_n%d.rds", r$key, SEED, N_BOOT)
  )
...
# Summary sheet is restricted to FOCAL_TERM — for Faith's PD / Hill
# phylogenetic (q=1, q=2), where it is quadratic, that row reports
# "quadratic — see per-response sheet" instead of a beta (plan §10; see the
# comment above the responses registry). Metadata records run provenance, and
...
# also not applicable for the three quadratic responses.
#
...
# The three quadratic responses report the same "quadratic — not applicable"
# string across every setting, so they never trigger this (n_distinct == 1).
...
if (nrow(sensitivity_inconsistent) > 0) {
  ...
  cat("\nSensitivity check: mineral nitrogen classification is robust to ROPE half-width",
      "(0.05-0.20) and to BCa vs percentile intervals for every response where it is",
      "applicable (Faith's PD / Hill phylogenetic q=1,q=2 are quadratic — not applicable).\n")
}
...
write_xlsx(
  ...
  path = "output/m_amf/coefficients_alpha_diversity.xlsx"
)
...
# state that beta is fully standardised (plan §9 "axis mismatch"). Faith's PD
# / Hill phylogenetic (q=1, q=2) get the beta1/beta2 annotation form
# automatically (get_boot_annotation() branches on whether a quadratic term
# exists for the parameter), unaffected by the Summary-sheet degradation above.
...
# Rename all objects with _endo suffix so they can be loaded alongside
# analogous g-am objects without name collisions
data_endo                      <- data

pred_richness_nitrogen_endo    <- pred_richness_nitrogen
annot_rich_nitrogen_endo       <- annot_rich_nitrogen
r2_richness_endo               <- boot_richness$r2
partial_r2_richness_endo       <- boot_richness$partial_r2

pred_abund_srs_nitrogen_endo   <- pred_abund_srs_nitrogen
annot_abund_srs_nitrogen_endo  <- annot_abund_srs_nitrogen
r2_abundance_endo              <- boot_abundance_srs$r2
partial_r2_abundance_endo      <- boot_abundance_srs$partial_r2

if (!dir.exists("generated_data")) dir.create("generated_data")

save(
  data_endo,
  pred_richness_nitrogen_endo,
  annot_rich_nitrogen_endo,
  r2_richness_endo,           partial_r2_richness_endo,
  pred_abund_srs_nitrogen_endo,
  annot_abund_srs_nitrogen_endo,
  r2_abundance_endo,          partial_r2_abundance_endo,
  file = "generated_data/figure_2_m_amf.Rdata"
)
cat("Figure 2 Densosporales data saved to generated_data/figure_2_m_amf.Rdata\n")

#### Figure s5 ####

figure_s5 <- wrap_plots(
...
cat("Results exported to: output/m_amf/coefficients_alpha_diversity.xlsx\n")
...
cat("Figure 2 data saved to: generated_data/figure_2_m_amf.Rdata\n")
