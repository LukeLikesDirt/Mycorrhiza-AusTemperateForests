...
otu_long_amf <- fread("data/amf/otu_table_srs.txt") %>%
  ...
  inner_join(
    taxa_amf %>% select(otu_id, genus, am_group),
    by = "otu_id"
  )
...
genus_summary_amf_raw <- list(
  abundance = otu_long_amf %>% summarise(abundance = sum(abundance), .by = c(genus, am_group)),
  richness = otu_long_amf %>% filter(abundance > 0) %>% summarise(abs_richness = n_distinct(otu_id), .by = c(genus, am_group))
) %>%
  reduce(inner_join, by = c("genus", "am_group")) %>%
  mutate(
    rel_abundance = abundance / sum(abundance),
    rel_richness = abs_richness / sum(abs_richness)
  ) %>%
  select(genus, am_group, n_otus = abs_richness, Abundance = rel_abundance, Richness = rel_richness) %>%
  arrange(desc(Richness)) %>%
  print(n = Inf)

# Mutate genera with relative abundance & richness < 2% to a group-specific "Other"
...
# OTU count of the genera it absorbed, not an individual genus's count.
collapse_am_group <- function(data, other_label) {
  data %>%
    mutate(
      genus = case_when(
        Abundance < 0.02 & Richness < 0.02 ~ other_label,
        TRUE ~ format_taxon_name(genus, italics = FALSE)
      )) %>%
    group_by(genus) %>%
    summarise(Abundance = sum(Abundance), Richness = sum(Richness), n_otus = sum(n_otus)) %>%
    ungroup() %>%
    arrange(desc(genus == other_label), Richness) %>%
    mutate(genus = paste0(genus, " [", n_otus, "]"))
}

g_amf_summary <- genus_summary_amf_raw %>%
  filter(am_group == "Glomeromycota") %>%
  collapse_am_group("Other G-AMF genera")

m_amf_summary <- genus_summary_amf_raw %>%
  filter(am_group == "Endogonomycetes") %>%
  collapse_am_group("Other M-AMF genera")
...
n_g_amf <- sum(!startsWith(g_amf_summary$genus, "Other G-AMF genera"))
n_m_amf <- sum(!startsWith(m_amf_summary$genus, "Other M-AMF genera"))

g_amf_summary$fill <- "grey60"
g_amf_summary$fill[!startsWith(g_amf_summary$genus, "Other G-AMF genera")] <-
  rev(paletteer::paletteer_c("grDevices::GnBu", n_g_amf))

m_amf_summary$fill <- "grey90"
m_amf_summary$fill[!startsWith(m_amf_summary$genus, "Other M-AMF genera")] <-
...
genus_summary_amf <- bind_rows(m_amf_summary, g_amf_summary) %>%
...
amf_fill_colors <- genus_summary_amf %>%
  distinct(genus, fill) %>%
  tibble::deframe()

# Create the plot
plot_amf_genus <- ggplot(
  genus_summary_amf,
  aes(x = metric, y = value, fill = genus)
) +
  ...
  guides(fill = guide_legend(ncol = 3, byrow = FALSE, reverse = TRUE)) +
  labs(
    x = NULL,
