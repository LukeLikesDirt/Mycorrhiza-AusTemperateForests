#!/usr/bin/env Rscript
# cluster_otus.R — Dynamic OTU clustering for ECM ASVs
#
...
#
# Usage:
#   Rscript utils/cluster_otus.R \
#     --asv_sequences    data/asv_sequences.fasta \
#     --asv_table        data/asv_table.txt \
#     --asv_classification data/asv_classification.txt \
#     --ref_classification data/ref_seqs/eukaryome_ITS.classification \
#     --ecm_clusters     tmp_clusters/ecm_clusters.txt \
#     --threads          10
...
option_list <- list(
  ...
  make_option(c("-g", "--ecm_clusters"),
              type = "character", default = "./tmp_clusters/ecm_clusters.txt",
              metavar = "FILE",
              help = "ECM clusters file produced by 05_classify_asvs.sh [default: %default]"),
  make_option("--threads",
  ...
  make_option("--ecm_genera",
              type = "character", default = "./data/ref_seqs/ecm_genera.txt",
              metavar = "FILE",
              help = "ECM genera file with lineage and exploration_type columns [default: %default]"),
  make_option("--fungal_traits",
              type = "character", default = "./data/ref_seqs/fungal_traits.txt",
              metavar = "FILE",
              help = "FungalTraits file with primary_lifestyle column [default: %default]")
)
...
opt <- parse_args(
  OptionParser(
    ...
    usage = "%prog [options]",
    description = paste(
      "Dynamic OTU clustering for ECM ASVs.",
...
if (!file.exists(opt$ecm_clusters)) {
  stop("ECM clusters file not found: ", opt$ecm_clusters,
       "\nPlease run 05_classify_asvs.sh first.")
}
...
taxa_cutoffs_file           <- opt$ecm_clusters
threads                     <- opt$threads
maxaccepts                  <- opt$maxaccepts
maxrejects                  <- opt$maxrejects
strand                      <- opt$strand
ecm_genera_file             <- opt$ecm_genera
fungal_traits_file          <- opt$fungal_traits

if (!strand %in% c("plus", "both")) stop("--strand must be 'plus' or 'both'.")
if (!file.exists(opt$ecm_genera))    stop("--ecm_genera file not found: ",    opt$ecm_genera)
if (!file.exists(opt$fungal_traits)) stop("--fungal_traits file not found: ", opt$fungal_traits)
...
ecm_genera_info <- fread(ecm_genera_file) %>%
  select(genus, lineage, exploration_type)

fungal_traits_info <- fread(fungal_traits_file) %>%
...
# Clustering hierarchy for ECM fungi
...
otu_classification <- pre_clustered_otus %>%
  ...
  select(otu_id, reference_id, reference_taxonomy,
         phylum, class, order, family, genus, species, rank, score, abundance) %>%
  mutate(
...
fwrite(otu_sample_matrix, "./tmp_clusters/otu_ecm_table.txt", sep = "\t")
fwrite(otu_classification, "./tmp_clusters/otu_ecm_classification.txt", sep = "\t")
writeXStringSet(otu_representative_sequences, "./tmp_clusters/otu_ecm_sequences.fasta")

# ── Save final combined outputs ───────────────────────────────────────────────

non_ecm_clusters <- fread("./tmp_clusters/non_ecm_clusters.txt") %>%
  mutate(otu_id = str_replace(otu_id, ";size=\\d+$", ""))

non_ecm_clusters %>%
  fwrite("./output/otu_classification_non_ecm.txt", sep = "\t")

# Genus-level OTU tables

# ECM: group by genus, join lineage and exploration_type from ecm_genera
ecm_genus_clusters <- otu_sample_matrix %>%
  pivot_longer(cols = -otu_id, names_to = "sample_id", values_to = "abundance") %>%
  filter(abundance > 0) %>%
  inner_join(otu_classification %>% select(otu_id, genus), by = "otu_id") %>%
...
ecm_genus_clusters %>%
  pivot_wider(names_from = sample_id, values_from = abundance, values_fill = 0,
              values_fn = sum) %>%
  left_join(ecm_genera_info, by = "genus") %>%
  relocate(genus, lineage, exploration_type) %>%
  fwrite("./output/otu_table_ecm_genus.txt", sep = "\t")

# All (ECM + non-ECM): group by genus, join primary_lifestyle from FungalTraits
non_ecm_genus_clusters <- asv_sample_matrix %>%
  rename(otu_id = OTU_ID) %>%
  pivot_longer(cols = -otu_id, names_to = "sample_id", values_to = "abundance") %>%
  filter(abundance > 0) %>%
  inner_join(non_ecm_clusters %>% select(otu_id, genus), by = "otu_id") %>%
  group_by(genus, sample_id) %>%
  summarise(abundance = sum(abundance), .groups = "drop") %>%
  arrange(desc(abundance))

bind_rows(non_ecm_genus_clusters, ecm_genus_clusters) %>%
  group_by(genus, sample_id) %>%
  summarise(abundance = sum(abundance), .groups = "drop") %>%
  pivot_wider(names_from = sample_id, values_from = abundance, values_fill = 0,
              values_fn = sum) %>%
  left_join(fungal_traits_info, by = "genus") %>%
  relocate(genus, primary_lifestyle) %>%
  fwrite("./output/otu_table_all_genus.txt", sep = "\t")